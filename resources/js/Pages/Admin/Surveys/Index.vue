...
const deleteSurvey = (surveyId) => {
  if (confirm('Are you sure you want to delete this survey?')) {
    form.delete(route('surveys.destroy', surveyId), {
      onSuccess: () => {
        console.log("Survey deleted successfully.");
      },
      onError: (errors) => {
        console.error("Error in survey deletion:", errors);
      }
    });
  }
};
              ...
              <tr v-for="survey in surveys.data" :key="survey.id" class="bg-white border-b">
                ...
                <td class="py-4 px-6">
                  <div class="flex space-x-4">
                    <Link :href="route('surveys.edit', survey.id)" class="bg-green-500 px-2 py-1 rounded text-white cursor-pointer">Edit</Link>
                    <button @click="deleteSurvey(survey.id)" class="bg-red-500 px-2 py-1 rounded text-white cursor-pointer">Delete</button>
                    <Link :href="route('surveys.manage.questions', survey.id)" class="bg-blue-500 px-2 py-1 rounded text-white cursor-pointer">Manage Questions</Link>
                  </div>
                </td>
              </tr>
            </tbody>
